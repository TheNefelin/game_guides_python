...
def test_auth_google_success(client, db):
  ...
  with patch("src.api.auth.google_service.verify_google_token", return_value=mock_info):
    response = client.post("/api/auth/google", json={"googleToken": "valid_token"})
...
def test_auth_google_unverified_email(client):
  mock_info = GoogleUserInfo(
    google_id="222",
...
def test_auth_google_invalid_token(client):
  with patch(
    ...
    response = client.post("/api/auth/google", json={"googleToken": "bad_token"})

  assert response.status_code == 401


def test_auth_google_existing_user(client, db):
...
def _create_session(db, user_id, days_valid=7, revoked=False):
  session = UserSession(
    user_id=user_id,
...
def test_auth_refresh_revoked_token(client, db):
  user = User(id=uuid.uuid4(), email="revoked@user.com", role_id=1)
  db.add(user)
  db.flush()
  session = _create_session(db, user.id, revoked=True)
...
def test_auth_refresh_expired_token(client, db):
  user = User(id=uuid.uuid4(), email="expired@user.com", role_id=1)
  db.add(user)
  db.flush()
  session = _create_session(db, user.id, days_valid=-1)  # expired yesterday
...
def test_auth_refresh_invalid_token(client):
  response = client.post("/api/auth/refresh", json={"refresh_token": "non_existent_token"})
  assert response.status_code == 401
...
def test_auth_logout_invalid_token(client):
  response = client.post("/api/auth/logout", json={"refresh_token": "bad_token"})
  assert response.status_code == 401
